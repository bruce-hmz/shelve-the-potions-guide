        ...
        <div className="container article-frame">
          ...
          <div className="article-body">
            ...
            <section id="performance" className="article-section">
              ...
              <p>
                A second honesty note, specific to the diagonal variant: settings may not fix it at all. A player in the &quot;Mouse Stuttering?&quot; thread whose jitter appears <strong>only on diagonal mouse movement</strong> systematically tested the whole settings matrix on 7 September — frame-rate cap, vsync, resolution, mouse and controller sensitivity, axis inversion, FOV, brightness, and all three window modes — and nothing improved the stutter. Their own suspect is that the game handles the mouse movement vector per axis rather than as one combined input, which would make this an input-handling bug rather than a settings problem; the developer is engaged in the thread. A 5 September Steam review from a trackball user separately describes severe view jitter and vertigo, so input devices may matter too. Until a patch addresses input handling, treat the vsync + FPS-cap advice above as the first thing to try — it addresses the frame-rate-tied subset — and consider the diagonal variant unresolved as of v1.0.12.
              </p>
              <p>
                <strong>If the problem is crashing rather than stuttering — specifically when you alt-tab or the window loses focus — that is a separate issue with its own first check.</strong> One player reported the game crashing whenever another window took focus, in fullscreen, borderless, and windowed mode alike. The developer&apos;s 6 September diagnostic found nothing unusual in that player&apos;s hardware, but noted, &quot;we had issues with game on Windows 10 updates that are lower than 21H1+&quot; — so if you are on Windows 10, <strong>update to 21H1 or newer first</strong>. If the crashes persist after that, send the developers your player log (found in %USERPROFILE%\AppData\LocalLow\Knight Owl Games\Shelve the Potions!) via their Discord. This is developer-advised troubleshooting from a single known report, not a confirmed universal fix.
              </p>
              <p className="source-note">
                Source: Steam threads &quot;Mouse Stuttering?&quot; (27 Aug 2026, developer reply; 7 Sep 2026 systematic settings test), &quot;Stuttering/choppiness?&quot; (31 Aug – 1 Sep 2026, developer reply), &quot;Weird mouse behaviour?&quot; (community frame-rate tests 7 Aug 2026, new report 1 Sep 2026), and &quot;Game Crashes on alt tab or unfocus window&quot; (4–6 Sep 2026, developer diagnostic), plus a 5 Sep 2026 Steam review from a trackball user. Not retested by us in-game.
              </p>
            </section>
            ...
            <section id="sources" className="article-section">
              ...
              <ul className="mistake-list" style={{ listStyle: "none" }}>
                ...
                <li>Crash on alt-tab / window unfocus: Steam thread &quot;Game Crashes on alt tab or unfocus window&quot; (4–6 Sep 2026), developer diagnostic — known issues on Windows 10 builds below 21H1, player log requested via Discord. Single known report.</li>
